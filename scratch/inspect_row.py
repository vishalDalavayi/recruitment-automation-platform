...
query = f"SELECT {cols}, i.vendor_name, 'active' as type, s.serial_no FROM scrapped_data.active_scraped_data s JOIN scrapped_data.input_active i ON s.keyword = i.dice_search_link LIMIT 1"
...
if row:
    ...
    print("No row found in active_scraped_data")
